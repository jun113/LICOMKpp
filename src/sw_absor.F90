...
!         if(chl_temp.lt.0.2D0)D0 chl_temp=0.2D0 D0 for  experiment 3 different
        ...
        chlindx = min(chlindx,nsub)         ! maximum limit for chl index (i,j)
         pen_chl(i,j,k,iblock)=Tr(k,chlindx)*OD0CP        !(i,j)
!         if(k.eq.1.and.chloro(i,j,iblock).gt.0.1.and.i.eq.10) &
!         write(*,*)'pen_chlsw',pen_chl(i,j,k,iblock)/OD0CP,&
!         pen_chl(i,j,k,iblock),chloro(i,j,iblock)
           END DO             
         END DO
         END DO 
        END DO 
