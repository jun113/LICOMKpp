...
// Overlap
extern double* bufSendOverlap;
extern double* bufRecvOverlap;
extern double* bufTripoleOverlap;

// MPI request ids
extern MPI_Request* sndRequestOverlap;
extern MPI_Request* rcvRequestOverlap;
// MPI status flags
extern MPI_Status* sndStatusOverlap;
extern MPI_Status* rcvStatusOverlap;
...
// POP_HaloUpdate3D
template<typename T>
void pop_halo_update_overlap_isend_irecv(T* const array, 
		const int &I2, const int &I1, const int &I0,
    const CppPOPHaloMod::POPHalo &halo,
    const int &fieldLoc, const int &fieldKind, const int &nn) {
  ...
  T* bufRecv3D    = static_cast<T *>(&(bufRecvOverlap[nn * bufSizeRecv * halo.numMsgRecv * I2]));
  T* bufSend3D    = static_cast<T *>(&(bufSendOverlap[nn * bufSizeSend * halo.numMsgSend * I2]));
  T* bufTripole3D = static_cast<T *>(&(bufTripoleOverlap[nn * (POP_HALO_WIDTH + 1) * IMT_GLOBAL * I2]));

  MPI_Status*  sndStatus  = &(sndStatusOverlap[nn * halo.numMsgSend]);
  MPI_Status*  rcvStatus  = &(rcvStatusOverlap[nn * halo.numMsgRecv]);

  MPI_Request* sndRequest = &(sndRequestOverlap[nn * halo.numMsgSend]);
  MPI_Request* rcvRequest = &(rcvRequestOverlap[nn * halo.numMsgRecv]);
...
template<typename T>
void pop_halo_update_overlap_wait(T* const array, 
		const int &I2, const int &I1, const int &I0,
    const CppPOPHaloMod::POPHalo &halo,
    const int &fieldLoc, const int &fieldKind, const int &nn) {
...
// 3D I2-priority, I1 I0 I2
template<typename T>
void pop_halo_update_priority_k_overlap_isend_irecv (T* const array,
		const int &I2, const int &I1, const int &I0,
    const CppPOPHaloMod::POPHalo &halo,
    const int &fieldLoc, const int &fieldKind, const int &nn) {
...
template<typename T>
void pop_halo_update_priority_k_overlap_wait (T* const array,
		const int &I2, const int &I1, const int &I0,
    const CppPOPHaloMod::POPHalo &halo,
    const int &fieldLoc, const int &fieldKind, const int &nn) {
