...
!Any comments please send to huangwenyu@mail.tsinghua.edu.cn                      
    ...
    use precision_mod
    use constant_mod
    use pconst_mod, only : iday,dzp, zkt
    use utils
    implicit none
...
1000    continue
    ...
    subroutine canuto_2010_interface(Km_out,    Kh_out,     Ks_out,     Kd_out,     mld_out,&
                                     ...
                                     n2_in,     s2_in,      lat_in,     lev_in,     num_lev,ii,jj,iblock, isc,uvstar)
...
!               if ( k < 3 ) TKE_mld = TKE_mld+uvstar*uvstar*78.63D0
!               TKE_mld = TKE_mld+uvstar*uvstar*3.75D0*exp(zkt(k)*4.0D0*PI/3.0D1)
...
!
            if (k < 4) then
                Km_out(k) = max(1.0D-3 , Km_out(k))
                Kh_out(k) = max(1.0D-3 , Kh_out(k))
                Ks_out(k) = max(1.0D-3 , Ks_out(k))
            else
                Km_out(k) = max(1.0D-4 , Km_out(k))
                Kh_out(k) = max(1.0D-5 , Kh_out(k))
                Ks_out(k) = max(1.0D-5 , Ks_out(k))
            end if
!
!
...
!
!YU  May 10th, 2017
                if ( n2_in(k) < very_small) then
                  kh_out(k) =min(8.0D-3*DZP(K)*DZP(K), 8.0D0)
                  ks_out(k) =min(8.0D-3*DZP(K)*DZP(K), 8.0D0)
                  km_out(k) =min(8.0D-3*DZP(K)*DZP(K), 8.0D0)
                end if
!YU  May 10th, 2017
!       print*,"!======================================================" , K, mld_lev
...
!               end if
    ...
    subroutine canuto_2010_interface_debug(Km_out,    Kh_out,     Ks_out,     Kd_out,     mld_out,&
                                     ts_in,     ss_in,      rho_in,     ri_in,      rrho_in,&
                                     n2_in,     s2_in,      lat_in,     lev_in,     num_lev,ii,jj,iblock, isc,uvstar,wka)
        ...
        ! if (mytid == 3) then
        !     write(*,*)jj,ii,wka(ii,jj,1),mld_lev
        ! endif
...
!
    ...
    end subroutine canuto_2010_interface_debug
