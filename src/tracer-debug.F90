...
!     =================
      SUBROUTINE TRACER_debug(in_n,wk_wkb,wk_wkc,wk_wkd,wk_stf,wk_tf,wk_adv_tt)
!     =================
#include <def-undef.h>
use precision_mod 
use param_mod
...
use hmix_del2
      ...
      integer     :: in_n
      real(r8),dimension(imt,jmt,km)::wk_wkb,wk_wkc,wk_wkd,wk_tf,wk_adv_tt
      real(r8),dimension(imt,jmt)::wk_stf
...
!  END DO
   DO iblock = 1, nblocks_clinic
      DO K = 1,KM
         DO J = 1, JMT
            DO I = 1,IMT
               WKb (I,J,K,IBLOCK)= wk_wkb(i,j,k)
               WKc (I,J,K,IBLOCK)= wk_wkc(i,j,k)
               WKD (I,J,K,IBLOCK)= wk_wkd(i,j,k)
               tf (I,J,K,IBLOCK)= wk_tf(i,j,k)
               adv_tt (I,J,K)= wk_adv_tt(i,j,k)
            END DO
         END DO
      END DO
  END DO
...
if (allocated(wkb)) then
      deallocate(wkb,wkc,wkd)
endif
if (allocated(rit)) then
      deallocate(rit)
endif
      RETURN
      END SUBROUTINE TRACER_debug
