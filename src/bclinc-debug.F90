...
!     =================
      SUBROUTINE BCLINC_debug(wk_dlu,wk_dlv)
!     =================
...
use operators
      ...
      IMPLICIT NONE
      REAL(r8),dimension(imt,jmt,km)    :: wk_dlu,wk_dlv
...
!---------------------------------------------------------------------
   DO IBLOCK = 1, NBLOCKS_CLINIC
      DO K = 1,KM
         DO J = 1,jmt
            DO I = 1,imt
               DLU (I,J,K,IBLOCK) = wk_dlu(i,j,k)
               DLv (I,J,K,IBLOCK) = wk_dlv(i,j,k)
            END DO
         END DO
      END DO
  END DO
